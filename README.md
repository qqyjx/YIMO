<div align="center">
  ...
  <p><strong>Smart Grid EAV Data Management & Semantic Deduplication Platform</strong></p>
  <p>
...
│
├── scripts/                  # 核心处理脚本
│   ├── eav_full.py           # Excel → EAV 导入（多表单支持）
...
│
├── webapp/                   # Flask Web 应用
│   ├── app.py                # 主应用（RAG + Deepseek API 代理）
│   ├── start_web.sh          # 启动脚本
│   ├── stop_web.sh           # 停止脚本
│   ├── templates/            # HTML 模板
│   ├── components/           # React/TSX 组件（可选前端）
│   ├── services/             # 服务层代码
│   └── static/               # 静态资源
│
├── dataset/                  # 原始数据文件
...
│
├── grideav/                  # Python 虚拟环境（不上传）
└── nocodb-data/              # NocoDB 数据（可选）
```
